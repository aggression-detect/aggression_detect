...
def createDataset(datasets_video_path, figure_output_path,fix_len, force = False):
    ...
    for dataset_name, dataset_video_path in datasets_video_path.items():
        ...
        dataset_images = []
        # print(dataset_video_path)
        for filename in os.listdir(dataset_video_path):
            #print(filename)
            if filename.endswith(".mp4") or filename.endswith(".mpg"):   #avi
                #print("get!!!")
    ...
    #print(avg_length)
    train_path, test_path, train_y, test_y =  train_test_split(videos_frames_paths,videos_labels, test_size=0.1, random_state=42)
    ...
    train_path, valid_path, train_y, valid_y = train_test_split(train_path, train_y, test_size=0.8, random_state=42)
    # print(np.shape(train_path))
    # print(np.shape(valid_path))
    
    valid_path = list(valid_path) + list(train_path) + list(test_path)
    valid_y = valid_y + train_y + test_y
